...
class QueryDatabricks(object):
    ...
    def __init__(self):
        ...
        self.aggr_dbr_monthly_table = "hive_metastore.default.aggr_dbr_monthly"
        self.aggr_dbr__daily_table = "hive_metastore.default.aggr_dbr__daily"
        ...
        try:
            sql = """
                    SELECT userId,vin,month_id,acceleration_cnt,deceleration_cnt,sharp_turn_cnt, 
                           ...
                           month_date 
                    from {0} 
                    where month_id>='{1}' and month_id<='{2}'
                    order by month_id desc,userId,vin;
                  """.format(self.aggr_dbr_monthly_table, month_start, month_end)
            # print(sql)
        ...
        try:
            sql = """
                    SELECT userId,vin,day_id,acceleration_cnt,deceleration_cnt,sharp_turn_cnt, 
                           ...
                           report_date 
                    from {0} 
                    where day_id>='{1}' and day_id<='{2}'and mileage>200
                    order by day_id desc,userId,vin;
                  """.format(self.aggr_dbr__daily_table, day_start, day_end)
            # print(sql)
    ...
    #                 FROM (
    #                     SELECT userId, vehicleId, name, status, timestamp, date_id,
    #                            ROW_NUMBER() OVER (PARTITION BY vehicleId
    #                            ORDER BY timestamp DESC) as row_num
    #                     FROM {0}
    #                     WHERE vehicleId='{1}' and status=1
    #                 )
    #                 WHERE row_num=1;
    #               """.format(self.vdaa_gdc_v2_table, vin)
    #         # print(sql)
    #
    #         self.cursor.execute(sql)
    #         ret = self.cursor.fetchall()
        ...
        try:
            ...
            # 3. redis没有，就从hive查询，然后保存到redis。
                ...
                if ret:
                    ...
                    # 3.2 设置到redis中
                    g.db_redis.setex(vin, 5, str(self.warning_lights))
                    # g.db_redis.set_name_value(vin, str(self.warning_lights))
                    # g.db_redis.expire(vin, 5)
